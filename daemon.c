...
static void offer_new_offer(void *data, void *offer, const char *mime) {
...
static struct offer_listener offer_listener = {
...
static void free_full_offer_void(void *full_offer_void) {
    ...
    data_control.offer_destroy(full_offer->offer);
...
static void device_data_offer(void *data, void *device, void *offer) {
    (void) device;
    ...
    *full_offer = (struct full_offer) {
    ...
    data_control.offer_add_listener(offer, &offer_listener, &full_offer->mimes);
}
...
static struct zzz_list process_offer(struct daemon_device_state *state, void *offer) {
...
static void receive_offer(const struct zzz_list *mimes_to_save, void *offer, int *recv_fds, int *send_fds) {
    ...
    ZZZ_LIST_FOREACH(*mimes_to_save, curr_mime) {
        int fds[2];
        pipe(fds);
        data_control.offer_receive(offer, curr_mime->value, fds[1]);
...
static void store_selection(struct daemon_device_state *state, void *offer) {
...
static void replace_selection(struct daemon_device_state *state, void *device) {
    ...
    if (safe_roundtrip(state)) {
        ...
        void *source = data_control.manager_create_data_source(state->wl_objs->manager);
        ZZZ_LIST_FOREACH(state->saved_items, curr_saved_item) {
            struct clip_item *item = curr_saved_item->value;
            data_control.source_offer(source, item->mime);
        }
        ...
        *saved_items_alloc = state->saved_items;
        state->saved_items = zzz_list_empty;
        data_control.source_add_listener(source, &source_listener, saved_items_alloc);
        data_control.device_set_selection(device, source);
    }
}
...
static void device_selection(void *data, void *device, void *offer) {
    struct daemon_device_state *state = data;
    state->tamper_count++;
...
static void device_primary_selection(void *data, void *device, void *offer) {
    (void) device;
    struct daemon_device_state *state = data;
...
static void device_finished(void *data, void *device) {
    struct daemon_device_state *state = data;

    data_control.device_destroy(device);
    state->wl_objs->device = NULL;
    zzz_list_free(&state->pending_offers, free_full_offer_void);
    state->pending_offers = zzz_list_empty;
}

static struct device_listener device_listener = {
...
void daemon_dcm_callback(void *data, struct wl_objs *wl_objs) {
    ...
    *state = (struct daemon_device_state) {
    ...
    data_control.device_add_listener(wl_objs->device, &device_listener, state);
}
